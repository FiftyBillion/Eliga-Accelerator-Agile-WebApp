  ...
  <v-container>
    <v-row>
      <v-col cols="6">
        ...
        <v-row class="mx-4">
          <v-col cols="12" v-for="item in sprint" :key="item.id">
            <v-card width="70%" class="center border-card" ripple @click="getThePB(item)" :class="{ active: active_sprint == item.id}">
      ...
      <v-col>
        ...
        <v-divider class="secondary darken-4 my-3"></v-divider>
        <v-container class="scroll">
        <v-row class="py-1 px-5" v-for="item in PBinSprint1" :key="item.id" justify="center">
...
export default {
  computed: {
    ...
    ...mapState("ProductBacklog", ["PBinSprint1"]),
    ...mapState("SprintBacklog", ["active_sprint"])
  },
  mounted() {
  ...
  methods: {
    ...
    ...mapActions("SprintBacklog", ["setActiveSprint"]),
    getThePB(sprint) {
      this.getPBInSprint(sprint);
      this.setActiveSprint(sprint.id);
    }
  }
};
</script>
